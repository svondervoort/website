    ...
    return (
        <div className="py-4 border-b border-dashed border-primary group-hover:opacity-50 hover:!opacity-100 group">
            ...
            <div className={'flex justify-between items-center'}>
                <span className={'text-sm text-primary border border-solid border-primary rounded inline-block px-2 mr-8'}>{item.level}</span>
                <div>
            ...
            {'' != item.description && <div className={'text-sm mt-2 border-t border-dashed border-primary pt-4 hidden group-hover:block'}>{item.description}</div>}
        </div>
    );
}
