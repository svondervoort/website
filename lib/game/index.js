...
// The letters sit in the site's palette: white at a spread of opacities rather than a
// spread of hues, the same white the timeline line and card borders are drawn in.
const textColors = [
...
// The one colour in the game, carried by the enemies and the particles they burst
// into. It is what marks a thing as clickable: in white they read as scenery next to
// the letters and the portrait behind them.
const accentColor = "#FFDD00";

const wordmark = "SVONDERVOORT";
...
export const createGame = (canvas) => {
  ...
  let letters = initLetters(canvas, wordmark.split(""), textColors);
  let enemies = initEnemies(canvas, accentColor);
  let particles = [];
  let score = 0;
  let won = false;
  ...
  const handlePointerDown = (event) => {
    ...
    particles = particles.concat(createMouseParticles(coords, accentColor));
    score += 1;
    won = score >= winScore;
  ...
  const loop = (time) => {
    ...
    if (!won) enemies = updateEnemies(canvas, enemies, score, accentColor);
