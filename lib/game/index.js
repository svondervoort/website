...
// White at a spread of opacities rather than a spread of hues: the letters, enemies
// and particles all draw from this, so the game sits in the same palette as the
// timeline and the card borders instead of introducing colours of its own.
const colors = [
...
export const createGame = (canvas) => {
  ...
  let enemies = initEnemies(canvas, colors);
  let particles = [];
  let score = 0;
  let won = false;
  ...
  const loop = (time) => {
    ...
    if (!won) enemies = updateEnemies(canvas, enemies, score, colors);
