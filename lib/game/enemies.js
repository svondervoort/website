...
const createEnemy = (canvas, color) => ({
...
export const initEnemies = (canvas, color) => {
  return Array.from({ length: minEnemies }, () => createEnemy(canvas, color));
};

export const updateEnemies = (canvas, enemies, score, color) => {
  ...
  while (alive.length < minEnemies) {
    alive.push(createEnemy(canvas, color));
  }
